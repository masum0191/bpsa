...
<div class="row">
    ...
    <a class="btn btn-primary btn-sm" href="{{url('admin/post/create')}}">  New Post</a> 
    </div>
</div>
...
<td>{!!@$order->user !!}</td>
   ...
   <a class="btn btn-danger btn-sm"  data-toggle="modal" data-target="#Publish{{@$order->id}}">Add Date</a>
   <div class="modal fade" id="Publish{{@$order->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
          ...
          <form action="{{url('admin/post/publish/')}}" method="post" enctype="multipart/form-data">
            @csrf
    ...
    <a class="btn btn-warning btn-sm"  data-toggle="modal" data-target="#Publish{{@$order->id}}">Change date</a>
   <div class="modal fade" id="Publish{{@$order->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
            <div class="modal-header">
 ...
 <td> 
    @if(@$order->status==1)
    <a class="btn btn-primary btn-sm" href="{{url('admin/post/sinnotice/'.@$order->id)}}"><i class="fa fa-solid fa-toggle-on"></i></a>
    @else
    <a class="btn btn-dark  btn-sm"  href="{{url('admin/post/sacnotice/'.@$order->id)}}"><i class="fa fa-thin fa-toggle-off"></i></a>
    @endif
    </td> 
     <td> 
    @if(@$order->notice==1)
    <a class="btn btn-primary btn-sm" href="{{url('admin/post/ninnotice/'.@$order->id)}}"><i class="fa fa-solid fa-toggle-on"></i></a>
    @else
    <a class="btn btn-dark  btn-sm"  href="{{url('admin/post/nacnotice/'.@$order->id)}}"><i class="fa fa-thin fa-toggle-off"></i></a>
    @endif
    </td> 
     <td> 
    @if(@$order->highlight==1)
    <a class="btn btn-primary btn-sm" href="{{url('admin/post/hinnotice/'.@$order->id)}}"><i class="fa fa-solid fa-toggle-on"></i></a>
    @else
    <a class="btn btn-dark  btn-sm"  href="{{url('admin/post/hacnotice/'.@$order->id)}}"><i class="fa fa-thin fa-toggle-off"></i></a>
    @endif
    </td> 
<td>

        <a class="btn btn-sm btn-primary"  href="{{url('admin/post/edit/'.@$order->id)}}"><i class="fa fa-edit"></i></a>
         <!--edit start Modal -->
...
<input type="hidden" name="id" class="form-control" value="{{$order->id}}" />
    ...
    <div class="form-group">
     ...
     <textarea name="Sub_Heading" id="" class="form-control textarea2" require>{!!$order->Sub_Heading!!}</textarea>
    </div>
    <div class="form-group">
    ...
    <div class="form-group nopadding">
     <label>Details</label>
     <textarea name="Details" class="form-control textarea" id="">{!!$order->Details!!}</textarea>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
     <input type="submit" name="send" class="btn btn-info" value="Submit" />
        ...
        <a class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this Post?');" href="{{url('admin/post/destroy/'.@$order->id)}}"><i class="fa fa-trash"></i></a>
       ...
       <!-- Modal -->
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
           ...
           <img src="{{@$order->Cover_Photo}}" style="height: 100px; width:100px;" alt="Responsive image">
             ...
             <div class="col-md-4">
             <form action="{{url('admin/post/remove/')}}" method="post" enctype="multipart/form-data">
            @csrf
          <div class="form-group">
